 ...
 *
 * Runs the user-supplied verify command (PI_AGILE_VERIFY_CMD) in the project
 * cwd. The command is executed through a shell (`sh -c` / `cmd /c`) so it may
 * be any command string with arguments ("npm test", "pytest -x", etc.). Exit
 * code 0 == passed; anything else (incl. timeout/kill) == failed.
 */
...
/**
 * Run the verification command. When PI_AGILE_VERIFY_CMD is unset the result is
 * a hard failure (verification is mandatory in v3.1), so the caller must not
